...
export const Gallery = ({ project, className }: GalleryProps) => {
  const galleryItems = useMemo<ProjectGalleryItem[]>(() => {
    ...
    if (project.video) items.push({ type: 'video' as const, url: project.video, title: project.title });

    if (project.image) items.push({ type: 'image' as const, url: project.image, title: project.title });
